  ...
  let call_nextsupport j vi s =
	...
	Fcl_data.Array.set s i (j::s.(i))
	  ...
	  match Fd.value v2 with
	    Unk v2_ -> Fd.remove v2 j
	  | Val vj -> assert (vj = j); Fcl_stak.fail "ac6" in
  let s = 
    let s = Array.create intervalv1 [] in
...
module HSet = struct
  ...
  let addmem h k = if not (Hashtbl.mem h k) then add h k
  let mem = Hashtbl.mem
  let iter f = Hashtbl.iter (fun k _ -> f k)
  let fold f = Hashtbl.fold (fun k _ -> f k)
...
let cstr6 v1 v2 goods =
  ...
  let create_ac v =
    ...
    {v6 = v;
     ...
     adj6 = Array.create range Fcl_domain.empty;
     sup6 = Array.create range [];
...
let rec cstr_soft v1 v2 goods =
  ...
  let create_ac v =
    ...
    {vs = v;
     ...
     adjs = Array.create range Fcl_domain.empty;
     sups = Array.create range [];
     val2idxs;
     nbgoods;
