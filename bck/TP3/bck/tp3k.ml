...
that takes the size of the big square and the array of the sizes of the small squares as
parameters and solves the tiling problem. This function must:
• define the domain such that the small squares remain inside the big one;
• constrain the small squares pairwise such that they don’t overlap, using reification
operators.
...
let solve = fun size t ->
  ...
  let increment = (* count all solutions then succeed *)
    Goals.atomic (fun () -> incr nb; fprint "tiled.txt"; Printf.printf "\nsol #%d\n%!" !nb) in
  ...
  let fprint = fun file -> (* impression gnuplot *)
    let ch = open_out file in
    Array.iteri
      (fun i ti ->
        let xi = Fd.elt_value x.(i) and yi = Fd.elt_value y.(i) in
        Printf.fprintf ch "%d %d\n%d %d\n%d %d\n%d %d\n%d %d\n\n"
          xi yi (xi+ti) yi (xi+ti) (yi+ti) xi (yi+ti) xi yi) t;
    close_out ch in
