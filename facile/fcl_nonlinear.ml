...
let min_max_abs_for_abs x =
  ...
  | Unk xa ->
      ...
      (* x à cheval sur 0 *)
      let a' = Fcl_domain.greatest_leq xa 0
      and b' = Fcl_domain.smallest_geq xa 0 in
      (min (-a') b', max (-a) b)
...
let min_max_of_expn x n =
  ...
  match Fd.value x with
	...
	if min_xa >= 0 then (min_xan, max_xan) (* positive domain *)
	  ...
	  let min_neg = Fcl_domain.greatest_leq xa 0
	  and min_pos = Fcl_domain.smallest_geq xa 0 in
	  let minn =
  ...
  and update _ =
	...
	Val a, _ -> Fd.unify z (expn_int a n); true
	  ...
	  try
	    ...
	    if even then Fd.refine_low_up x (-root) root
