...
Event occuring when a variable is instantiated.
 ...
 (* Symmetry breaking *)
  Cstr.post (fd2e m.(0).(0) <~ fd2e m.(n-1).(n-1)); (* sym vs 2nd diagonale  *)
  Cstr.post (fd2e m.(0).(0) <~ fd2e m.(0).(n-1));  (* sym vs mediane verticale  *)
  Cstr.post (fd2e m.(0).(0) <~ fd2e m.(n-1).(0));   (* sym vs mediane horizontale  *)
  Cstr.post (fd2e m.(n-1).(0) <~ fd2e m.(0).(n-1));  (* sym vs 1ere diagonale  *)

(* redundant cstr *)
  (* sum equals n(n^2+1)/2 *)
  let s2 = n * (n * n + 1) / 2 in
  Cstr.post (sum =~ i2e s2);
...
instantiated) variable in the array fds for the criterion order. *)
  ...
  let print_bt = fun bt -> Printf.printf " backtracks \r%d" bt in
...
returned.
 ...
 (*
  ...
  let goal = labeling &&~ goal_print(* &&~ Goals.fail*) in
  let print_bt bt = printf " backtracks \r%d%!" bt in
